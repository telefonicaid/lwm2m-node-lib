...
function createRegisterResponseHandler(host, port, callback) {
    return function responseHandler(res) {
        if (res.code === '2.01') {
            logger.debug(context, 'Registration succeeded to host [%s] and port [%s]', host, port);
            ...
            for (var i = 0; i < res.options.length; i++) {
                if (res.options[i].name === 'Location-Path') {
                    deviceInformation.location = location;
                }
            }

            startListener(deviceInformation, callback);
        } else {
...
function register(host, port, url, endpointName, callback) {
    ...
    req.on('error', function (error) {
        logger.debug(context, 'Registration response finished with an error:\n%s', error);
