...
function startCoap(config, callback) {
    ...
    if ( typeof(config.dtls) !== 'undefined' ){
      var dtlsPort = config.dtls.port;
      var dtlsCrt = config.dtls.cert;
      var dtlsKey = config.dtls.key;
      var dtlsProxy = require('node-dtls-proxy');
      var dgram = require('dgram');
      logger.debug(context, 'Starting DTLS COAP Server on port ['+dtlsPort+']');
      try {
        var dtls = dtlsProxy.createDTLSServer(dtlsCrt,dtlsKey,dtlsPort,'udp4');
        var dtlsSocket;
        var endpointPort;
