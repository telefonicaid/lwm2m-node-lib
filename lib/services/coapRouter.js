...
function dataHandler(serverInfo) {
    return function(req, res) {
        logger.debug('Handling request with method [%s] on url [%s]', req.method, req.url);
...
function startCoap(config, callback) {
    ...
    logger.info('Starting COAP Server on port [%d]', config.port);
    ...
    serverInfo.server.listen(config.port, function (error) {
        if (error) {
            logger.error('Couldn\'t start COAP server: %s', error);
        } else {
            logger.info('COAP Server started successfully');
        }
...
function setHandler(serverInfo, type, handler) {
    logger.debug('Setting [%s] handler', type);
