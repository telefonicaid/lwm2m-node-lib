...
function startCoap(config, callback) {
    ...
    if ( typeof(config.dtls) != "undefined" ){
      var dtls_port = config.dtls.port;
      var dtls_crt = config.dtls.cert;
      var dtls_key = config.dtls.key;
      const dtls_proxy = require('node-dtls-proxy');
      const dgram = require('dgram');
      logger.debug(context, 'Starting DTLS COAP Server on port ['+dtls_port+']');
      try {
        const dtls = dtls_proxy.createDTLSServer(dtls_crt,dtls_key,dtls_port,"udp4");
        var dtls_socket;
        var endpointPort;
        ...
        dtls.on( 'secureConnection', function( socket ) {
          ...
          udp.on('message', function (msg, rinfo) {
             logger.debug('UDP2DTLS Connection: '+rinfo.port);
             endpointPort = rinfo.port
             dtls_socket.send(msg);
          });
          endpointPort = 5683
          logger.debug(context, 'DTLS Server received a connection');
          socket.on( 'message', function( message ) {
            dtls_socket = socket
            udp.send(message, 0, message.length, endpointPort, endpoint)
          });
        });
        logger.info(context, 'COAP DTLS Server started successfully');
      }
      catch(e){
        logger.error(context, 'Couldn\'t start DTLS COAP server: %s', e);
      }
    }
}
