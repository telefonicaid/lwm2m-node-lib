...
function read(deviceId, objectType, objectId, resourceId, callback) {
    ...
    function processResponse(res, callback) {
        if (res.code === '2.05') {
            ...
            callback(new errors.ResourceNotFound('/' + objectType + '/' + objectId));
        } else {
...
function writeAttributes(deviceId, objectType, objectId, resourceId, attributes, callback) {
    ...
    function processResponse(res, innerCb) {
        if (res.code === '2.04') {
            innerCb(null, res.payload.toString('utf8'));
        } else if (res.code === '4.04') {
            innerCb(new errors.ObjectNotFound('/' + objectType + '/' + objectId));
        } else {
            innerCb(new errors.ClientError(res.code));
        }
    }
...
function generateProcessResponse(objectType, objectId, code) {
...
function discover(deviceId, objectType, objectId, resourceId, callback) {
    ...
    async.waterfall([
        ...
        createReadRequest,
        sendRequest,
        generateProcessResponse(objectType, objectId, '2.05')
    ], callback);
}
