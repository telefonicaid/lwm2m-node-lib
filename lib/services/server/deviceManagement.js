...
function sendRequest(request, callback) {
    ...
    logger.debug(context, 'Sending the following COAP Request:\n%s', JSON.stringify(request, null, 4));
...
function generateProcessResponse(objectType, objectId, resourceId, code) {
    return function processResponse(res, callback) {
        if (res.code === code) {
            ...
            logger.debug(context, 'Resource "/%s/%s/%s" not found', objectType, objectId, resourceId);

            callback(new errors.ResourceNotFound(objectId, objectType, resourceId));
        } else {
            logger.debug(context, 'Client error retrieving resource "/%s/%s/%s"', objectType, objectId, resourceId);
