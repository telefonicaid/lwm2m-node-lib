...
function endRegistration(req, res) {
    return function (error, result) {
        if (error) {
            logger.debug('Registration request ended up in error [%s] with code [%s]', error.name, error.code);
            ...
            logger.debug('Registration request ended successfully');
            res.code = '2.01';
...
function applyHandler(queryParams, payload, handler, callback) {
    logger.debug('Calling user handler for registration actions for device [%s]', queryParams.ep);
...
function storeDevice(queryParams, req, callback) {
    ...
    logger.debug('Storing the following device in the db:\n%s', JSON.stringify(device, null, 4));
    ...
    if (device.type) {
        logger.debug('Registered device [%s] with type [%s]', device.name, device.type);
        registry.register(device, callback);
    } else {
        logger.debug('No type found for device [%s]', device.name);
