...
function endUnregistration(req, res) {
    return function (error, result) {
        if (error) {
            logger.debug('Registration request ended up in error [%s] with code [%s]', error.message, error.code);

            res.code = error.code;
            res.end(error.message);
        } else {
            logger.debug('Unregistration request ended successfully');
...
function applyHandler(handler, removedObj, callback) {
    logger.debug('Calling user handler for unregistration actions for device [%s]', removedObj.name);
...
function handleUnregistration(req, res, handler) {
    logger.debug('Handling unregistration request');
