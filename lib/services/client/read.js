    ...
    var data = {
        ...
        listener: function newModification(method, id, value) {
            if (method === 'setResource' && resourceId === id.toString()) {
                logger.debug('Sending data update to the server for object [%s]', data.objectUri);
                ...
                try {
                    ...
                    cancelObservation(storedObject.objectUri, resourceId, function() {
                        logger.info('Observation of [%s/%s] cancelled', storedObject.objectUri, resourceId);
                    });
                }
            }
        }
