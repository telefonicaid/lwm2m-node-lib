...
function ResourceObserver(stream, resourceId, storedObject) {
    var data = {
        ...
        listener: function newModification(method, id, value) {
            if (method === 'setResource' && resourceId === id) {
                data.observation++;
...
function discoverObjectResources(objectUri, callback) {
    objectRegistry.get(objectUri, function handleGetObject(error, obj) {
        if (error) {
            callback(error);
        } else {
            var payload = '<' + objectUri + '>',
                attributes = _.keys(obj.attributes);

            for (var i = 0; i < attributes.length; i++) {
                payload += ',<' + objectUri + '/' + attributes[i] + '>';
            }
...
function discoverObjectTypeInstances(objectTypeUri, callback) {
    function addPair(previous, value) {
        return previous + ';' + value[0] + '=' + value[1];
    }
    ...
    async.series([
        ...
        if (error) {
            ...
            if (attributes) {
                payload += _.pairs(attributes).reduce(addPair, '');
            }
