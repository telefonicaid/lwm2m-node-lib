...
function extractQueryParams(req, callback) {
    ...
    logger.debug('Extracting query parameters from request');
    ...
    if (req.urlObj.query) {
        logger.debug('Processing query [%s]', req.urlObj.query);
...
function checkMandatoryQueryParams(mandatoryQueryParams, queryParams, callback) {
    ...
    logger.debug('Checking for the existence of the following parameters [%j]', mandatoryQueryParams);
    ...
    if (missing.length !== 0) {
        ...
        logger.debug('Missing parameters found [%j]', missing);
        callback(error);
    } else {
        callback();
    }
